...
private:
    ...
    void CheckBatteryStatus() {
        ...
        // If battery level data is insufficient, read battery ADC again
        if (adc_values_.size() < kBatteryAdcDataCount) {
            ReadBatteryAdcData();
            return;
        }

        // If we have enough battery data, read it every kBatteryAdcInterval ticks
        ticks_++;
    ...
    void ReadBatteryAdcData() {
        ...
        // Append the ADC value to the queue
        ...
        // Above the maximum
            ...
            // Linearly interpolate intermediate value
            for (int i = 0; i < 5; i++) {
        ...
        // Check if we've reached the low battery threshold
        if (adc_values_.size() >= kBatteryAdcDataCount) {
    ...
    void ReadTemperature() {
        ...
        if (abs(temperature - current_temperature_) >= 3.5f) {  // Trigger callback only if temperature changes by more than 3.5°C
...
public:
    ...
    bool IsCharging() {
        // If the battery is already full, don't show charging
    ...
    bool IsDischarging() {
        // There's no separate discharging signal; return the inverse of charging
        return !is_charging_;
    }
