...
void LvglDisplay::UpdateStatusBar(bool update_all) {
    ...
    // Update mute icon
    {
        ...
        // If mute state changed, update the icon
    ...
    if (board.GetBatteryLevel(battery_level, charging, discharging)) {
        ...
        if (low_battery_popup_ != nullptr) {
            if (strcmp(icon, FONT_AWESOME_BATTERY_EMPTY) == 0 && discharging) {
                if (lv_obj_has_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN)) { // If low battery popup is hidden, show it
                    lv_obj_remove_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN);
                    app.PlaySound(Lang::Sounds::OGG_LOW_BATTERY);
                }
            } else {
                // Hide the low battery popup when the battery is not empty
                if (!lv_obj_has_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN)) { // If low battery popup is visible, hide it
    ...
    if (update_all || seconds_counter++ % 10 == 0) {
        // During firmware upgrade, don't read 4G network status to avoid occupying UART resources
